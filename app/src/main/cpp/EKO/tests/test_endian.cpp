...
static void test_roundtrip_u16() {
  ...
  auto le = eko::core::host_to_le(v);
  auto rt = eko::core::le_to_host(le);
...
static void test_le_to_host_from_bytes_u32() {
  ...
  auto host = eko::core::le_to_host(le_val);
...
static void test_zero() {
  check_eq("zero_u32", eko::core::le_to_host(std::uint32_t{0}), std::uint32_t{0});
  check_eq("zero_host_to_le_u32", eko::core::host_to_le(std::uint32_t{0}), std::uint32_t{0});
}

static void test_all_ones() {
  constexpr std::uint32_t all = 0xFFFFFFFF;
  check_eq("all_ones_le_to_host", eko::core::le_to_host(all), all);
  check_eq("all_ones_host_to_le", eko::core::host_to_le(all), all);
}

// ----- constexpr check -----

static void test_constexpr() {
  constexpr auto v = eko::core::le_to_host(std::uint32_t{0x12345678});
