...
using eko::app::Md5Xxh3CacheEntry;
...
static std::filesystem::path unique_test_dir() {
  ...
  return std::filesystem::temp_directory_path() / ("eko-md5-xxh3-" + std::to_string(stamp));
}
...
static void test_roundtrip_save_load() {
  ...
  const auto cache_file = eko::app::md5_xxh3_cache_file(dir);

  std::vector<Md5Xxh3CacheEntry> entries;
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x10), 1024, 0x1111222233334444ULL);
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x20), 2048, 0x5555666677778888ULL);

  auto save_st = eko::app::save_md5_xxh3_cache(cache_file, entries);
  if (!save_st) {
  ...
  auto loaded = eko::app::load_md5_xxh3_cache(cache_file);
  if (!loaded) {
  ...
  auto hit = eko::app::lookup_md5_xxh3_cache(*loaded, make_md5(0x20), 2048);
...
static void test_eviction_keeps_most_recent_65535() {
  ...
  for (std::uint64_t i = 0; i < 65540; ++i) {
    eko::app::remember_md5_xxh3_cache(entries, make_md5(static_cast<unsigned char>(i)), i, 0xABC00000ULL + i);
  }
  ...
  if (eko::app::lookup_md5_xxh3_cache(entries, make_md5(0), 0).has_value()) {
    fail_msg("eviction_keeps_most_recent_65535", "oldest entry was not evicted");
    return;
  }

  auto newest =
      eko::app::lookup_md5_xxh3_cache(entries, make_md5(static_cast<unsigned char>(65539)), 65539);
...
static void test_remember_updates_existing_pair() {
  ...
  eko::app::remember_md5_xxh3_cache(entries, md5, 4096, 0x1111ULL);
  eko::app::remember_md5_xxh3_cache(entries, md5, 4096, 0x2222ULL);
  ...
  auto hit = eko::app::lookup_md5_xxh3_cache(entries, md5, 4096);
...
static void test_forget_removes_existing_pair() {
  ...
  eko::app::remember_md5_xxh3_cache(entries, md5, 4096, 0x1111ULL);
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x67), 8192, 0x2222ULL);

  if (!eko::app::forget_md5_xxh3_cache(entries, md5, 4096)) {
    fail_msg("forget_removes_existing_pair", "existing cache entry was not removed");
    return;
  }

  if (eko::app::lookup_md5_xxh3_cache(entries, md5, 4096).has_value()) {
...
static void test_corrupt_primary_falls_back_to_backup() {
  ...
  std::vector<Md5Xxh3CacheEntry> entries;
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x44), 8192, 0xABCDEF1234567890ULL);
  ...
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x55), 16384, 0x0123456789ABCDEFULL);
  save_st = eko::app::save_md5_xxh3_cache(cache_file, entries);
  if (!save_st) {
    fail_msg("corrupt_primary_falls_back_to_backup", save_st.error());
    std::filesystem::remove_all(dir);
    return;
  ...
  auto hit = eko::app::lookup_md5_xxh3_cache(*loaded, make_md5(0x44), 8192);
...
static void test_locked_cache_file_fails_fast() {
  ...
  std::vector<Md5Xxh3CacheEntry> entries;
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x77), 4096, 0xABCDEF1234567890ULL);
  ...
  eko::app::remember_md5_xxh3_cache(entries, make_md5(0x78), 8192, 0x0123456789ABCDEFULL);
  save_st = eko::app::save_md5_xxh3_cache(cache_file, entries);
  CloseHandle(h);
