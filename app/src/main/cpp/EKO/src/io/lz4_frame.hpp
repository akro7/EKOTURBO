...
class Lz4BlockStreamReader {
 public:
  static eko::core::Result<Lz4BlockStreamReader> open(std::unique_ptr<ByteSource> src) noexcept;
  ...
  eko::core::Result<std::size_t> read_n_blocks(std::size_t n, std::vector<std::byte>& out) noexcept;

 private:
  Lz4BlockStreamReader(std::unique_ptr<ByteSource> src, Lz4FrameHeaderInfo hdr) noexcept
      : src_(std::move(src)), hdr_(hdr) {}

  eko::core::Status read_exact_(std::span<std::byte> out) noexcept;
...
class Lz4DecompressedSource final : public ByteSource {
 public:
  static eko::core::Result<std::unique_ptr<ByteSource>> open(std::unique_ptr<ByteSource> src) noexcept;
  ...
  eko::core::Status status() const noexcept override { return st_; }

 private:
  Lz4DecompressedSource(std::unique_ptr<ByteSource> src, Lz4FrameHeaderInfo hdr) noexcept;

  eko::core::Status read_exact_(std::span<std::byte> out) noexcept;
  eko::core::Status fill_next_block_() noexcept;
