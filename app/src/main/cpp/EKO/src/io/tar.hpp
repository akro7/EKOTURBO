...
class TarArchive {
 public:
  static eko::core::Result<TarArchive> open(std::string path, bool validate_header_checksums = true) noexcept;
 ...
 private:
  ...
  static eko::core::Result<std::uint64_t> parse_tar_number(const char* p, std::size_t n) noexcept;
  ...
  static eko::core::Result<PaxKV> parse_pax_payload(std::string_view payload) noexcept;

  eko::core::Status scan_() noexcept;
