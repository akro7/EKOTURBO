...
static eko::core::Result<std::uint64_t> parse_u64_dec(std::string_view s) noexcept {
  ...
  if (ec != std::errc{} || ptr != s.data() + s.size()) return eko::core::fail("PAX: invalid decimal number");
  return v;
}

} // namespace

eko::core::Result<TarArchive> TarArchive::open(std::string path, bool validate_header_checksums) noexcept {
  TarArchive t;
...
eko::core::Result<std::uint64_t> TarArchive::parse_tar_number(const char* p, std::size_t n) noexcept {
  ...
  if (b0 & 0x80) {
    ...
    if (negative) return eko::core::fail("Tar: negative base-256 numeric field");

    std::uint64_t val = static_cast<std::uint64_t>(b0 & 0x3Fu);
    for (std::size_t i = 1; i < n; ++i) {
      if (val > (std::numeric_limits<std::uint64_t>::max() >> 8))
        return eko::core::fail("Tar: base-256 numeric field too large for uint64");
...
eko::core::Result<TarArchive::PaxKV> TarArchive::parse_pax_payload(std::string_view payload) noexcept {
  PaxKV kv;
  ...
  while (pos < payload.size()) {
    ...
    if (!rl) return eko::core::fail(std::move(rl.error()));
    ...
    if (key == "path")
      ...
      if (!sz) return eko::core::fail(std::move(sz.error()));
      kv.size = *sz;
    }
  }
...
eko::core::Status TarArchive::scan_() noexcept {
  std::ifstream in(path_, std::ios::binary);
  if (!in.is_open()) return eko::core::failf("TarArchive: cannot open: {}", path_);
  ...
  auto read_exact = [&](std::byte* dst, std::size_t n) -> eko::core::Status {
    in.read(reinterpret_cast<char*>(dst), static_cast<std::streamsize>(n));
    if (!in.good()) return eko::core::failf("TarArchive: short read: {}", path_);
    pos += n;
    return {};
  };

  auto skip_exact = [&](std::uint64_t n) -> eko::core::Status {
    if (n == 0) return {};
    if (n > static_cast<std::uint64_t>(std::numeric_limits<std::streamoff>::max()))
      return eko::core::fail("TarArchive: entry too large for seekg");
    in.seekg(static_cast<std::streamoff>(n), std::ios::cur);
    if (!in.good()) return eko::core::failf("TarArchive: seek failed: {}", path_);
    pos += n;
    return {};
  };
  ...
  for (;;) {
    ...
    if (validate_ && !validate_header_checksum(std::span<const std::byte, 512>(header))) {
      return eko::core::failf("TarArchive: invalid header checksum in: {}", path_);
    }
    ...
    if (!szr) return eko::core::fail(std::move(szr.error()));
    std::uint64_t size = *szr;

    if (typeflag == 'x' || typeflag == 'g') {
      if (size > (1024ull * 1024ull * 8ull)) return eko::core::fail("TarArchive: refusing huge PAX header");
      ...
      if (!kvr) return eko::core::fail(std::move(kvr.error()));
    ...
    if (typeflag == 'L') {
      if (size > (1024ull * 1024ull * 8ull)) return eko::core::fail("TarArchive: refusing huge GNU longname header");
