...
eko::core::Result<Lz4FrameHeaderInfo> parse_lz4_frame_header(ByteSource& src) noexcept {
  ...
  if (magic != kLz4Magic) return eko::core::fail("LZ4: bad magic (not standard LZ4 frame)");
  ...
  if (version != 1) return eko::core::fail("LZ4: unsupported frame version");
  ...
  if (!info.block_independence) return eko::core::fail("LZ4: frame must use independent blocks");
  if (info.block_checksum) return eko::core::fail("LZ4: block checksum not supported");
  if (info.has_dict_id) return eko::core::fail("LZ4: dictionary ID not supported");
  if (!info.has_content_size) return eko::core::fail("LZ4: content size missing (compress with --content-size)");

  info.max_block_size = max_block_size_from_bd(info.bd);
  if (info.max_block_size == 0) return eko::core::fail("LZ4: invalid BD/max block size");
  if (info.max_block_size > static_cast<std::size_t>(LZ4_ONE_MIB))
    return eko::core::fail("LZ4: max block size > 1MiB not supported");
  ...
  if (info.content_size > LZ4_ONE_MIB && info.max_block_size != static_cast<std::size_t>(LZ4_ONE_MIB)) {
    return eko::core::fail("LZ4: content > 1MiB requires 1MiB blocks (compress with -B6)");
  }
...
eko::core::Result<Lz4BlockStreamReader> Lz4BlockStreamReader::open(std::unique_ptr<ByteSource> src) noexcept {
  if (!src) return eko::core::fail("LZ4: null source");
...
eko::core::Status Lz4BlockStreamReader::read_exact_(std::span<std::byte> out) noexcept {
  return read_exact(*src_, out);
}

eko::core::Result<std::size_t> Lz4BlockStreamReader::read_n_blocks(std::size_t n,
                                                                      std::vector<std::byte>& out) noexcept {
  if (n == 0) return std::size_t{0};

  const std::size_t before = out.size();
  const std::size_t total = total_blocks_1m();
  if (blocks_read_ + n > total) return eko::core::fail("LZ4: too many blocks requested");
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    if (raw_sz == 0) return eko::core::fail("LZ4: encountered endmark unexpectedly");
...
eko::core::Result<std::unique_ptr<ByteSource>> Lz4DecompressedSource::open(std::unique_ptr<ByteSource> src) noexcept {
  if (!src) return eko::core::fail("LZ4: null source");
  BRK_TRYV(h, parse_lz4_frame_header(*src));
  return std::unique_ptr<ByteSource>(new Lz4DecompressedSource(std::move(src), h));
}

eko::core::Status Lz4DecompressedSource::read_exact_(std::span<std::byte> out) noexcept {
  return read_exact(*src_, out);
}

eko::core::Status Lz4DecompressedSource::fill_next_block_() noexcept {
  if (produced_ >= total_out_) return eko::core::fail("LZ4: internal: produced >= total");
  if (!st_) return st_;
  ...
  const std::uint32_t raw_sz = u32_le(std::span<const std::byte, 4>(szb.data(), 4));
  if (raw_sz == 0) return eko::core::fail("LZ4: encountered endmark unexpectedly while decoding");
  ...
  if (uncompressed) {
    if (payload != expected_out) return eko::core::fail("LZ4: uncompressed block size mismatch");
    ...
    if (ret < 0) return eko::core::fail("LZ4: decompression failed (LZ4_decompress_safe)");
    if (static_cast<std::size_t>(ret) != expected_out)
      return eko::core::fail("LZ4: decompression produced unexpected size");
  }
...
eko::core::Result<std::unique_ptr<ByteSource>> open_lz4_decompressed(std::unique_ptr<ByteSource> src) noexcept {
