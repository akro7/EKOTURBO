...
eko::core::Status TcpListener::bind_and_listen(std::string bind_ip, std::uint16_t port, int backlog) noexcept {
  ...
  if (!fd_.valid()) {
    const int e = errno;
    return eko::core::failf("socket: {}", std::strerror(e));
  }
  ...
  if (do_bind(fd_, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) != 0) {
    const int e = errno;
    fd_.close();
    return eko::core::failf("bind: {}", std::strerror(e));
  }
  if (do_listen(fd_, backlog) != 0) {
    const int e = errno;
    fd_.close();
    return eko::core::failf("listen: {}", std::strerror(e));
  }
...
eko::core::Result<TcpConnection> TcpListener::accept_one() noexcept {
  if (!fd_.valid()) return eko::core::fail("TcpListener: not listening");
  ...
  for (;;) {
    ...
    if (pr < 0) {
      const int e = errno;
      if (e == EINTR) continue;
      if (e == EBADF) return eko::core::fail("accept: listener closed");
      return eko::core::failf("poll: {}", std::strerror(e));
    }
  ...
  for (;;) {
    ...
    if (cfd >= 0) {
      ...
      if (!ip) {
        const int e = errno;
        ::close(cfd);
        return eko::core::failf("inet_ntop: {}", std::strerror(e));
      }
    ...
    const int e = errno;
    if (e == EINTR) continue;
    if (e == EAGAIN || e == EWOULDBLOCK) return eko::core::fail("accept: timeout");
    if (e == EBADF || e == EINVAL) return eko::core::fail("accept: listener closed");
    return eko::core::failf("accept: {}", std::strerror(e));
  }
}
