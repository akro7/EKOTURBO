...
static eko::core::Status fail_iokit(const char* what, kern_return_t kr) noexcept {
  return eko::core::failf("{}: IOKit error 0x{:08x}", what, static_cast<unsigned>(kr));
}
...
eko::core::Status UsbFsDevice::open_and_init() noexcept {
  ...
  if (!loc) return eko::core::failf("Invalid macOS USB locationID sysname: '{}'", devnode_);

  io_service_t service = find_device_by_location(*loc);
  if (!service) return eko::core::failf("USB device not found at location: {}", devnode_);
  ...
  if (res != S_OK || !devIntf) return eko::core::fail("Failed to get IOUSBDeviceInterface320");
  dev_intf_ = devIntf;
  ...
  if (!ifc_intf_) {
    close();
    return eko::core::fail("No interface with bulk endpoints found");
  }
