...
static eko::core::Status fail_errno(const char* what) noexcept {
  const int e = errno;
  return eko::core::failf("{}: {}", what, std::strerror(e));
}
...
eko::core::Status UsbFsDevice::open_and_init() noexcept {
  close();
  ...
  if (!writable_) {
    close();
    return eko::core::fail("UsbFsDevice: opened read-only");
  }
...
eko::core::Status UsbFsDevice::detach_kernel_driver_() noexcept {
...
eko::core::Status UsbFsDevice::claim_interface_() noexcept {
  if (!fd_.valid() || ifc_num_ < 0) return eko::core::fail("UsbFsDevice: no interface to claim");
  int ifc = ifc_num_;
...
eko::core::Status UsbFsDevice::parse_descriptors_() noexcept {
  if (!fd_.valid()) return eko::core::fail("UsbFsDevice: fd invalid");
  ...
  if (buf.size() < USB_DT_DEVICE_SIZE) return eko::core::fail("UsbFsDevice: missing device descriptor");
  ...
  if (dev->bLength < USB_DT_DEVICE_SIZE || dev->bDescriptorType != USB_DT_DEVICE)
    return eko::core::fail("UsbFsDevice: invalid device descriptor");

  ids_.vendor = dev->idVendor;
  ids_.product = dev->idProduct;

  std::size_t off = dev->bLength;
  if (off + USB_DT_CONFIG_SIZE > buf.size()) return eko::core::fail("UsbFsDevice: missing config descriptor");
  ...
  if (cfg->bLength < USB_DT_CONFIG_SIZE || cfg->bDescriptorType != USB_DT_CONFIG)
    return eko::core::fail("UsbFsDevice: invalid config descriptor");

  const std::size_t cfg_off = off;
  const std::size_t cfg_total = cfg->wTotalLength;
  if (cfg_total < cfg->bLength) return eko::core::fail("UsbFsDevice: invalid wTotalLength");
  if (cfg_off + cfg_total > buf.size()) return eko::core::fail("UsbFsDevice: config exceeds read data");
  ...
  while (off + 2 <= end) {
    ...
    if (bType == USB_DT_INTERFACE) {
      ...
      if (bLength < USB_DT_INTERFACE_SIZE) return eko::core::fail("UsbFsDevice: short interface descriptor");
      ...
      // FIX: Removed premature ifc_num_ assignment here.
      // Previously, setting ifc_num_ on class-10 (CDC Data) before verifying bulk
      // endpoints caused claim_interface_() to grab the wrong interface — the CDC
      // control interface has no bulk endpoints, so PIT exchange always failed.
      // commit_ifc() below already does the correct assignment: it only sets
      // ifc_num_ after confirming both bulk_in and bulk_out are present.
    } else if (bType == USB_DT_ENDPOINT) {
      if (bLength < USB_DT_ENDPOINT_SIZE) return eko::core::fail("UsbFsDevice: short endpoint descriptor");
  ...
  if (eps_.bulk_in == 0 || eps_.bulk_out == 0) return eko::core::fail("UsbFsDevice: missing bulk endpoints");
  return {};
}
