...
eko::core::Status TcpListener::bind_and_listen(std::string bind_ip, std::uint16_t port, int backlog) noexcept {
  if (!wsa_init_) {
    WsaScope::acquire();
    wsa_init_ = true;
  }
  ...
  if (fd_ == INVALID_SOCKET) return eko::core::failf("socket failed: {}", WSAGetLastError());
  ...
  if (::InetPtonA(AF_INET, bind_ip_.c_str(), &addr.sin_addr) != 1)
    return eko::core::failf("Invalid bind ip: {}", bind_ip_);

  if (::bind(fd_, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR)
    return eko::core::failf("bind failed: {}", WSAGetLastError());
  if (::listen(fd_, backlog) == SOCKET_ERROR) return eko::core::failf("listen failed: {}", WSAGetLastError());

  spdlog::debug("TcpListener: listening on {}:{}", bind_ip_, port_);
  return {};
}

eko::core::Result<TcpConnection> TcpListener::accept_one() noexcept {
  if (fd_ == INVALID_SOCKET) return eko::core::fail("TcpListener: not listening");
  ...
  if (pr == 0) return eko::core::fail("accept: timeout");
  if (pr == SOCKET_ERROR) {
    const int err = WSAGetLastError();
    if (err == WSAEINTR) return eko::core::fail("accept: timeout");
    return eko::core::failf("poll: {}", err);
  }
  if (pfd.revents & (POLLERR | POLLHUP | POLLNVAL)) return eko::core::fail("accept: listener closed");
  ...
  if (cfd == INVALID_SOCKET) {
    const int err = WSAGetLastError();
    if (err == WSAEWOULDBLOCK) return eko::core::fail("accept: timeout");
    return eko::core::failf("accept failed: {}", err);
  }
  ...
  if (!ip) {
    ::closesocket(cfd);
    return eko::core::failf("inet_ntop failed: {}", WSAGetLastError());
  }
