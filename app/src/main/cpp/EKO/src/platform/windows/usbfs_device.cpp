...
eko::core::Status UsbFsDevice::open_and_init() noexcept {
  ...
  if (handle_ == INVALID_HANDLE_VALUE) {
    ...
    return eko::core::fail("Failed to open COM port '" + port_path + "': " + win32_err(e));
  }
  ...
  if (!::GetCommState(handle_, &dcb)) {
    const DWORD e = ::GetLastError();
    close();
    return eko::core::fail("GetCommState failed: " + win32_err(e));
  }
  ...
  if (!::SetCommState(handle_, &dcb)) {
    const DWORD e = ::GetLastError();
    close();
    return eko::core::fail("SetCommState failed: " + win32_err(e));
  }
