...
constexpr std::string_view kCacheHeader = "eko-md5-xxh3-cache v1";
...
#if defined(_WIN32)
eko::core::Status ensure_replaceable_now(const std::filesystem::path& path) noexcept {
  std::error_code ec;
  if (!std::filesystem::exists(path, ec)) {
    if (ec) return eko::core::failf("Cannot inspect cache file {}: {}", path.string(), ec.message());
    return {};
  }
  if (ec) return eko::core::failf("Cannot inspect cache file {}: {}", path.string(), ec.message());
  ...
  if (err == ERROR_SHARING_VIOLATION || err == ERROR_LOCK_VIOLATION || err == ERROR_ACCESS_DENIED) {
    return eko::core::failf("Cache file busy, skipping save: {}", path.string());
  }
  if (err == ERROR_FILE_NOT_FOUND || err == ERROR_PATH_NOT_FOUND) return {};
  return eko::core::failf("Cannot access cache file {} for replace: {}", path.string(), err);
}
#endif

eko::core::Result<ParsedCacheFile> parse_cache_file(const std::filesystem::path& path) noexcept {
  std::error_code ec;
  if (!std::filesystem::exists(path, ec)) return ParsedCacheFile{};
  if (ec) return eko::core::failf("Cannot access cache file {}: {}", path.string(), ec.message());

  const auto size = std::filesystem::file_size(path, ec);
  if (ec) return eko::core::failf("Cannot stat cache file {}: {}", path.string(), ec.message());
  ...
  if (!in.is_open()) return eko::core::failf("Cannot open cache file {}", path.string());
  ...
  if (!in.eof() && in.fail()) return eko::core::failf("Cannot read cache file {}", path.string());
...
eko::core::Status replace_cache_file(const std::filesystem::path& src,
  ...
  if (!ReplaceFileW(dst.c_str(), src.c_str(), backup.c_str(), REPLACEFILE_IGNORE_MERGE_ERRORS, nullptr, nullptr)) {
    ...
    if (replace_error == ERROR_FILE_NOT_FOUND || replace_error == ERROR_PATH_NOT_FOUND) {
      ...
      return eko::core::failf("Cannot move cache file {} into place: {}", dst.string(), GetLastError());
    }
    return eko::core::failf("Cannot replace cache file {}: {}", dst.string(), replace_error);
  }
  return {};
#else
  std::error_code ec;
  if (std::filesystem::exists(dst, ec) && !ec) {
    ...
    if (ec) return eko::core::failf("Cannot back up cache file {}: {}", backup.string(), ec.message());
  } else if (ec) {
    return eko::core::failf("Cannot inspect cache file {}: {}", dst.string(), ec.message());
  }

  std::filesystem::rename(src, dst, ec);
  if (ec) return eko::core::failf("Cannot replace cache file {}: {}", dst.string(), ec.message());
  return {};
#endif
}
...
eko::core::Result<std::vector<Md5Xxh3CacheEntry>> load_md5_xxh3_cache(
  ...
  for (const auto& candidate : candidates) {
    ...
    if (!parsed) return eko::core::fail(std::move(parsed.error()));
...
eko::core::Status save_md5_xxh3_cache(const std::filesystem::path& cache_file,
                                         std::vector<Md5Xxh3CacheEntry> entries,
  ...
  if (ec) return eko::core::failf("Cannot create cache directory {}: {}", parent.string(), ec.message());
...

  {
    ...
    if (!out.is_open()) return eko::core::failf("Cannot write cache file {}", tmp_file.string());
    ...
    out.flush();
    if (!out.good()) return eko::core::failf("Cannot flush cache file {}", tmp_file.string());
  }
  ...
  if (!verify_tmp) {
    ...
    return eko::core::fail(std::move(verify_tmp.error()));
  }
  if (!verify_tmp->has_header || verify_tmp->saw_corruption) {
    std::error_code rm_ec;
    std::filesystem::remove(tmp_file, rm_ec);
    return eko::core::failf("Refusing to install malformed cache file {}", tmp_file.string());
  }
