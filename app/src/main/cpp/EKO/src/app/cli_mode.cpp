...
struct Provider {
  std::vector<std::unique_ptr<eko::odin::UsbTarget>> usb;
  std::vector<eko::odin::Target> owned;
  std::vector<eko::odin::Target*> ptrs;
  std::unique_ptr<eko::platform::TcpConnection> wireless_conn;
};
...
eko::core::Result<CliArgs> parse_cli_args(int argc, char* argv[]) {
  CliArgs out;

  auto require_value = [&](int& i, const char* flag) -> eko::core::Result<std::string> {
    if (i + 1 >= argc) return eko::core::fail(std::string("Missing value for ") + flag);
    ++i;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    return eko::core::fail("Unknown argument: " + arg);
  }

  return out;
}

std::vector<eko::platform::UsbDeviceSysfsInfo> enumerate_samsung_targets() {
  eko::platform::EnumerateFilter f{.vendor = kSamsungVid};
  return eko::platform::enumerate_usb_devices_sysfs(f);
}

std::optional<eko::platform::UsbDeviceSysfsInfo> select_samsung_target(std::string_view sysname) {
  if (sysname.empty()) return std::nullopt;
  auto info = eko::platform::find_by_sysname(sysname);
  if (!info || info->vendor != kSamsungVid) return std::nullopt;
  return info;
}

std::optional<eko::platform::UsbDeviceSysfsInfo> select_odin_target(std::string_view sysname) {
...
bool is_pit_name(std::string_view base) noexcept {
  return eko::core::ends_with_ci(base, ".pit");
}

std::shared_ptr<const std::vector<std::byte>> pit_from_specs(
    const std::vector<eko::odin::ImageSpec>& specs) {
  const eko::odin::ImageSpec* pit = nullptr;
...
eko::core::Result<Provider> make_provider(const CliArgs& args, const eko::odin::Cfg& cfg) {
  Provider p;

  if (args.wireless) {
    eko::platform::TcpListener listener;
    ...
    for (;;) {
      ...
      if (ar) {
        p.wireless_conn = std::make_unique<eko::platform::TcpConnection>(std::move(*ar));
        break;
      }
      if (ar.error() != "accept: timeout") return eko::core::fail(std::move(ar.error()));
      std::this_thread::sleep_for(std::chrono::milliseconds(200));
    }

    p.owned.push_back(eko::odin::Target{.id = "wireless", .link = p.wireless_conn.get()});
    p.ptrs.push_back(&p.owned.back());
    return p;
  }

  std::vector<eko::platform::UsbDeviceSysfsInfo> targets;
  if (args.target && !args.target->empty()) {
    auto info = select_samsung_target(*args.target);
    if (!info) return eko::core::fail("Target sysname not found.");
    if (!is_odin_product(info->product)) {
      return eko::core::fail("The device is not in Odin Mode, reboot to download mode first.");
    }
    targets.push_back(*info);
  } else {
    const auto all_samsung = enumerate_samsung_targets();
    if (all_samsung.empty()) return eko::core::fail("No connected devices detected.");
    ...
    if (targets.empty()) return eko::core::fail("None of the devices are in Odin Mode");
  }
  ...
  for (const auto& td : targets) {
    auto ut = std::make_unique<eko::odin::UsbTarget>(td.devnode());

    auto st = ut->dev.open_and_init();
    if (!st) return eko::core::fail(std::move(st.error()));

    auto cst = ut->conn.open();
    if (!cst) return eko::core::fail(std::move(cst.error()));

    ut->conn.set_timeout_ms(cfg.preflash_timeout_ms);

    p.owned.push_back(eko::odin::Target{.id = ut->devnode, .link = &ut->conn});
    p.ptrs.push_back(&p.owned.back());
...
int run_flash_cli(const CliArgs& args) {
  ...
  std::optional<eko::core::SignalShield> sig_guard;
  ...
  ui.on_stage = [&](const std::string& s) {
    ...
    sig_guard = eko::core::SignalShield::enable([](const char* sig_desc, int count) {
  ...
  std::vector<eko::odin::ImageSpec> specs;
  if (!inputs.empty()) {
    auto jobsr = eko::app::md5_jobs(inputs);
    if (!jobsr) {
    ...
    auto vst = eko::app::md5_verify(*jobsr, ui);
    if (!vst) {
      spdlog::error("{}", vst.error());
      return 1;
    }

    auto specsr = eko::odin::expand_inputs_tar_or_raw(inputs);
    if (!specsr) {
    ...
    std::vector<eko::odin::ImageSpec> filtered;
  ...
  auto fst = eko::odin::flash(provider.ptrs, specs, pit_to_upload, cfg, ui);
  if (!fst) {
...
int run_cli(int argc, char* argv[]) {
  ...
  auto lock = eko::platform::SingleInstanceLock::try_acquire("eko-engine");
  if (!lock) {
