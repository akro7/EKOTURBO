...
inline void response_from_le(ResponseBox& r) noexcept {
  r.id = eko::core::le_to_host(r.id);
  r.ack = eko::core::le_to_host(r.ack);
}
...
inline RequestBox make_request(RqtCommandType type, RqtCommandParam param, std::span<const std::int32_t> ints = {},
  ...
  RequestBox r{};
  r.id = eko::core::host_to_le(static_cast<std::int32_t>(type));
  r.data = eko::core::host_to_le(static_cast<std::int32_t>(param));

  if (!ints.empty()) {
    const auto n = (ints.size() > RequestBox::DATA_INT_SIZE) ? RequestBox::DATA_INT_SIZE : ints.size();
    for (std::size_t i = 0; i < n; ++i) r.intData[i] = eko::core::host_to_le(ints[i]);
  }
