 ...
 *
 * CHANGE vs original:
 *   coordinator lambda → setpriority(PRIO_PROCESS, 0, -8) عند البداية
 *
 *   السبب: الـ coordinator هو الـ thread الأكثر أهمية أثناء الـ flash —
 *   يشغّل الـ prefetcher fill loop، يقرأ من الـ disk/LZ4، يدير الـ
 *   barrier sync مع الـ workers، ويرسل Steps للأجهزة. بدون priority
 *   boost، الـ scheduler ممكن يؤجّله أثناء busy workers، مما يسبب
 *   bubbles في USB pipeline.
 *
 *   الـ workers لديهم بالفعل nice=-8. الـ prefetcher reader thread
 *   يأخذها أيضًا من prefetcher.hpp. الـ coordinator كان الوحيد الناقص.
 */
...
struct FirstError {
  std::mutex m;
  std::optional<eko::core::Error> e;

  void set(eko::core::Status s) noexcept {
    if (s) return;
    std::lock_guard lk(m);
    if (e) return;
    e = std::move(s.error());
  ...
  eko::core::Status status_or_ok() noexcept {
    std::lock_guard lk(m);
    return e ? eko::core::Status{std::unexpect, *e} : eko::core::Status{};
  }
...
static eko::core::IByteTransport& link(Target& d) { return *d.link; }
...
static eko::core::Result<std::vector<ImageSpec>> sources_common_mapping_or_empty(
  ...
  for (const auto& s : sources) {
    ...
    for (auto* d : devs) {
      ...
      if (p->id != ref->id || p->dev_type != ref->dev_type)
        return eko::core::fail("PIT mapping differs across devices");
    }
...
static eko::core::Status send_prefetched(PF& pf, std::barrier<>& sync, Step& cur, const std::size_t pkt,
  ...
  auto pst = pf.status();
  return pst ? eko::core::Status{} : pst;
}

} // namespace

eko::core::Status flash(std::vector<Target*>& devs, const std::vector<ImageSpec>& sources,
                           std::shared_ptr<const std::vector<std::byte>> pit_to_upload, const Cfg& cfg,
                           Ui ui) noexcept {
  if (devs.empty()) return eko::core::fail("flash: no devices");
  for (auto* d : devs)
    if (!d || !d->link || !d->link->connected()) return eko::core::fail("flash: transport not connected");

  const bool has_sources = !sources.empty();
  const bool has_pit = pit_to_upload && !pit_to_upload->empty();

  if (!has_sources && !has_pit) return eko::core::fail("flash: nothing to do (no sources, no PIT)");
  ...
  auto fanout_keep = [&](auto&& fn) -> eko::core::Status {
    if (active.empty()) return eko::core::fail("No active devices");

    std::vector<eko::core::Status> sts(active.size(), eko::core::Status{});
    ...
    if (active.empty()) {
      ...
      if (st) st = eko::core::fail("All devices failed");
      return st;
    }
  ...
  auto finish = [&](eko::core::Status st, bool call_done_always) -> eko::core::Status {
    if (!st) {
    ...
    return (failed_total > 0 || first_err.has_error())
               ...
               : (ui.on_done ? (ui.on_done(), eko::core::Status{}) : eko::core::Status{});
  };
  ...
  auto shutdown_active = [&](OdinCommands::ShutdownMode m, std::string_view stg) -> eko::core::Status {
    log_shutdown_action(m);
    stage(stg);
    return fanout_keep([&](Target& d) { return OdinCommands(link(d)).shutdown(m); });
  };

  auto steps = std::vector<std::move_only_function<eko::core::Status()>>{};

  steps.emplace_back([&] {
    stage(stage_label::kHandshake);

    auto st = fanout_keep([&](Target& d) -> eko::core::Status {
      auto& c = link(d);
  ...
  steps.emplace_back([&] -> eko::core::Status {
    if (active.empty()) return eko::core::fail("No active devices");
  ...
  steps.emplace_back([&] {
    ...
    return fanout_keep([&](Target& d) -> eko::core::Status {
  ...
  steps.emplace_back([&] -> eko::core::Status {
    ...
    if (ref.empty()) return eko::core::fail("PIT cpu_bl_id missing");
    for (auto* d : active)
      if (d->pit_table.cpu_bl_id != ref) return eko::core::fail("cpu_bl_id mismatch across devices");
  ...
  steps.emplace_back([&] -> eko::core::Status {
    ...
    if (!ndevs) return eko::core::fail("No active devices");
    ...
    auto exec = [&](OdinCommands& odin, const Step& s) -> eko::core::Status {
    ...
    // ── coordinator ───────────────────────────────────────────────────────
    // يعمل على الـ calling thread (JNI bridge thread من FlashService).
    // يحتاج priority boost لأنه يتحكم في الـ prefetcher fill + barrier sync:
    //   - كل تأخير هنا = bubble في USB pipeline على جميع الأجهزة
    //   - الـ workers والـ prefetcher reader لديهم بالفعل nice=-8
    //   - الـ coordinator كان الوحيد الناقص
    auto coordinator = [&]() -> eko::core::Status {
#ifdef __ANDROID__
      setpriority(PRIO_PROCESS, 0, -8);
#endif
      u64 overall_done = 0;
      ...
      for (std::size_t idx = 0; idx < items.size(); ++idx) {
        ...
        if (item.spec.lz4 && use_lz4) {
          ...
          if (!total_decomp) return eko::core::fail("LZ4 content size is zero: " + item.spec.display);

          const std::size_t max_blocks = detail::lz4_nonfinal_block_limit(cfg.buffer_bytes);
          if (!max_blocks)
            return eko::core::fail("buffer_bytes too small for compressed download (needs >= 1MiB)");

          u64 sent = 0;

          eko::core::TwoSlotPrefetcher<Slot> pf(
              [&](Slot& s, std::stop_token stt) -> eko::core::Result<bool> {
          ...
          if (!file_sz) return eko::core::fail("Empty source: " + item.spec.display);
          ...
          eko::core::TwoSlotPrefetcher<Slot> pf(
              [&](Slot& s, std::stop_token stt) -> eko::core::Result<bool> {
                ...
                if (!rst) return eko::core::fail(std::move(rst.error()));
