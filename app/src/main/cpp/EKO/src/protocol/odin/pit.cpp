...
eko::core::Result<PitTable> parse(std::span<const std::byte> bytes) noexcept {
  if (bytes.size() < sizeof(PitHeaderWire)) return eko::core::fail("PIT parse: buffer too small for header");

  PitHeaderWire hdr{};
  std::memcpy(&hdr, bytes.data(), sizeof(hdr));

  hdr.magic = eko::core::le_to_host(hdr.magic);
  hdr.count = eko::core::le_to_host(hdr.count);
  hdr.lu_count = eko::core::le_to_host(hdr.lu_count);
  hdr.reserved = eko::core::le_to_host(hdr.reserved);

  if (hdr.magic != PIT_MAGIC) return eko::core::fail("PIT parse: bad magic");
  if (hdr.count < 0) return eko::core::fail("PIT parse: negative partition count");
  ...
  if (bytes.size() < required) return eko::core::fail("PIT parse: buffer smaller than declared partition table");
  ...
  for (std::size_t i = 0; i < count; ++i, off += sizeof(PartitionInfoWire)) {
    ...
    w.binType = eko::core::le_to_host(w.binType);
    w.devType = eko::core::le_to_host(w.devType);
    w.id = eko::core::le_to_host(w.id);
    w.attribute = eko::core::le_to_host(w.attribute);
    w.updateAttribute = eko::core::le_to_host(w.updateAttribute);
    w.blockSize = eko::core::le_to_host(w.blockSize);
    w.blockLength = eko::core::le_to_host(w.blockLength);
    w.offset = eko::core::le_to_host(w.offset);
    w.fileSize = eko::core::le_to_host(w.fileSize);
