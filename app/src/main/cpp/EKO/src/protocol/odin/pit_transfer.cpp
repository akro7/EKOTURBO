...
eko::core::Result<std::vector<std::byte>> download_pit_bytes(OdinCommands& odin, unsigned retries) noexcept {
  BRK_TRYV(sz, odin.get_pit_size(retries));

  if (sz <= 0) return eko::core::fail("Device returned invalid PIT size");
...
eko::core::Result<pit::PitTable> download_pit_table(OdinCommands& odin, unsigned retries) noexcept {
